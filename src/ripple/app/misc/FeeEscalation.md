...
for the queue if it meets these additional criteria:
  ...
  * all other queued transactions for that account, in the case where
  they spend the maximum possible STM, leave enough STM balance to pay
  the fee,
  * the total fees for the other queued transactions are less than both
  the network's minimum reserve and the account's STM balance, and
...
This demonstrates that a simpler transaction paying less STM can be more
likely to get into the open ledger, or be sorted earlier in the queue
than a more complex transaction paying more STM.
