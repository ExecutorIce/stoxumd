 ...
 * By using STM for bridging the most complicated path expected is usually:
 * source -> gateway -> STM -> gateway -> destination
3. Pareto principle.
 * Good liquidity is often reusable.
...
If you're sending and receiving STM you don't need a path.
...
2. Build a payment transaction containing the path set.
  ...
  * A payment transaction includes the following fields
    ...
    * destination amount
      ...
      * if currency not STM, must have a specified issuer.
